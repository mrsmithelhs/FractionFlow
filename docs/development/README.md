...
|---|---|---|---|
| `plan-01` | Node Toolchain and Static Deployment Spike | delivered | Stand up the minimal Node-based dev toolchain (package manifest, lockfile, build step, headless test runner), decide and prove the static GitHub Pages deployment mechanism with a harmless published smoke check, and ratify the repository source/build-output layout. This spike proves deployment plumbing only; it does not prove any learner-facing behavior. |
