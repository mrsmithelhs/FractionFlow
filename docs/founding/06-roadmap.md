...
Core interaction works through the intended access modes.
...
The bridge should ask the learner to participate in the correspondence rather than merely watch it.
...
The project should now formalize independent support dimensions such as:
...
# 59. Phase 8 — Local Progress and Continuity
...
Returning learners should experience sensible continuity without the site becoming a progress dashboard.
...
This could preserve the static-first architecture while allowing targeted practice.
...
Cloud synchronization may eventually be valuable when learners use multiple devices.

It should enter only after there is evidence that local progress is valuable enough to synchronize.
...
A backend is a product commitment, not merely a technical upgrade.
